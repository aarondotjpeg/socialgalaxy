...
if($check > 0){
	if($row['status'] != 'Completed'){
		...
		if($_POST['payment_status'] == 'Completed'){
			mysql_query("UPDATE tumblr_account SET time_travel_credit = (time_travel_credit + ".$credit.")  WHERE tumblr_username = '".$_POST['option_selection2']."'");
		}
	}
} else {
	mysql_query("INSERT INTO paypal (`username`,`paypal_id`,`credit_type`,`credit_qty`,`auth`,`status`) VALUES ('".$_POST['option_selection2']."','".$_POST['payer_id']."','ttc',".$credit.",'".$_POST['txn_id']."','".$_POST['payment_status']."')");
}